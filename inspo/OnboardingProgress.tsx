...
const OnboardingProgress: React.FC<OnboardingProgressProps> = ({ companyId }) => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        ...
        // Add debugging logs
        console.log("OnboardingProgress - Token available:", !!token);
        console.log("OnboardingProgress - Company ID:", companyId);
        
        if (!token) {
          throw new Error('Authentication required');
        }
        
        // Use relative URL with proxy instead of environment variable
        const apiUrl = `/api/companies/${companyId}`;
        console.log("OnboardingProgress - Fetching company from:", apiUrl);
        ...
        console.log("OnboardingProgress - API response status:", response.status);
        ...
        console.log("OnboardingProgress - Company data received:", companyData);
        ...
        console.error("OnboardingProgress - Error details:", err);
  ...
  const updateStepStatus = async (stepId: string, newStatus: 'todo' | 'in_progress' | 'done') => {
    try {
      ...
      // Use relative URL with proxy instead of environment variable
      const apiUrl = `/api/companies/${companyId}/onboarding/${stepId}`;
      console.log("Updating step status at:", apiUrl);
      ...
      console.log("Update step status response:", response.status);
